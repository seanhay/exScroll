...
function exScroll(opts = {}) {
	...
	containers.forEach((container) => {
		...
		const elVisiblePercentage: number =
			(elVisibleWidth / elScrollWidth) * 100
		...
		function scrollStartHandler(event: TouchEvent | MouseEvent) {
			isScrolling = true
			scrollStart =
		...
		function scrollMoveHandler(event: TouchEvent | MouseEvent) {
			if (isScrolling) {
				...
				const scrollOffsetPercent: number =
					((scrollPos - scrollStart) / scrollTrack.offsetWidth) * 100

				if (
					scrollPercent + scrollOffsetPercent >= 0 &&
					scrollPercent + scrollOffsetPercent <=
						100 - elVisiblePercentage
				) {
					// While is inside container bounds
					scrollDragger.style.cssText += `; left: ${
						scrollPercent + scrollOffsetPercent
					}%`
					el.scrollLeft =
